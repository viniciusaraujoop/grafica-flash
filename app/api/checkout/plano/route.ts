...
async function getCompany(
  ...
  if (requester?.id) {
    ...
    if (ownerCompany?.id) return { company: ownerCompany, requester, canManage: true }
    ...
    try {
      ...
      if (member?.company_id) {
        ...
        const canManage = ['dono', 'gerente', 'admin', 'super_admin'].includes(role)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!mpToken) {
      return NextResponse.json(
        { error: 'MERCADO_PAGO_ACCESS_TOKEN não configurado na Vercel ou no .env.local.' },
        { status: 500 }
      )
    }
    ...
    if (paymentError || !payment?.id) {
      return NextResponse.json(
        { error: paymentError?.message || 'Erro ao registrar pagamento em plan_payments.' },
        { status: 500 }
      )
    }
    ...
    if (!mpResponse.ok) {
      ...
      return NextResponse.json(
        {
          error: mpData.message || mpData.error || 'Erro do Mercado Pago ao criar preferência.',
          details: mpData,
