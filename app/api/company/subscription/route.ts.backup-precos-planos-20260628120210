...
function normalizarAction(value: unknown): Action {
  if (value === 'cancel' || value === 'sync' || value === 'renew' || value === 'create') return value
  return 'create'
}
...
async function syncSubscriptionData(
  ...
  const currentSubscriptionId = subscriptionId || company?.mercado_pago_subscription_id

  if (!currentSubscriptionId) {
    return { company, subscription: null }
  }

  const subscription = await mercadoPagoRequest(`/preapproval/${currentSubscriptionId}`)

  const status = subscription?.status || 'desconhecido'
  const nextPaymentDate = calcularProximaCobranca(subscription?.next_payment_date || null)
  const plano = normalizarPlano(company?.assinatura_plano || company?.plano || 'profissional')
  const update: Record<string, unknown> = {
    ...
    assinatura_checkout_url: subscription.init_point || subscription.sandbox_init_point || company.assinatura_checkout_url || null,
  ...
  if (status === 'authorized') {
    ...
    update.assinatura_inicio = company.assinatura_inicio || new Date().toISOString()
    ...
    update.assinatura_status = assinaturaEstaAtiva(company) ? 'ativa' : 'pendente'
    update.assinatura_auto_recorrente = false
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!access.canManage) {
      return NextResponse.json(
        { error: 'Apenas o dono da empresa ou gerente pode gerenciar a assinatura.' },
        { status: 403 }
      )
    }
    ...
    const planoId = normalizarPlano(body.plano || access.company.assinatura_plano || access.company.plano || 'profissional')
    const plano = planos[planoId]
    const siteUrl = getSiteUrl()
    const email = String(body.email || access.company.email || requester.email || '').trim().toLowerCase()
    ...
    const { data: updatedCompany, error: companyError } = await supabaseAdmin
      ...
      .update({
        ...
        assinatura_status: assinaturaEstaAtiva(access.company) ? 'ativa' : 'pendente',
