...
async function mercadoPagoRequest(path: string) {
  ...
  if (!response.ok) {
    throw new Error(data?.message || data?.error || `Erro Mercado Pago ${response.status} em ${path}`)
  }
...
function calcularNovaExpiracao(expiracaoAtual: string | null, nextPaymentDate?: string | null) {
  if (nextPaymentDate) {
...
async function updateCompanyBySubscription(
  ...
  const plano = normalizarPlano(paymentRow?.plano || empresa?.assinatura_plano || empresa?.plano)
  ...
  const companyUpdate: Record<string, unknown> = {
    ...
    assinatura_checkout_url: subscription?.init_point || subscription?.sandbox_init_point || null,
  ...
  if (paymentRow?.id) {
    await supabaseAdmin
      ...
      .update({
        ...
        checkout_url: subscription?.init_point || subscription?.sandbox_init_point || null,
        raw_webhook: body,
...
async function processSubscriptionWebhook(body: any, resourceId: string) {
  ...
  const result = await updateCompanyBySubscription(supabaseAdmin, subscription, body)
...
async function processAuthorizedPaymentWebhook(body: any, resourceId: string) {
  const supabaseAdmin = getSupabaseAdmin()
  const authorizedPayment = await mercadoPagoRequest(`/authorized_payments/${resourceId}`)
  const subscriptionId = authorizedPayment?.preapproval_id
  const paymentStatus = authorizedPayment?.payment?.status || authorizedPayment?.status
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (topic.includes('subscription_authorized_payment')) {
      const result = await processAuthorizedPaymentWebhook(body, String(resourceId))
      return NextResponse.json(result)
    }
