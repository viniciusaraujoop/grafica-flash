...
if (-not (Test-Path -LiteralPath ".git" -PathType Container)) {
    throw "Coloque este arquivo diretamente na raiz do projeto grafica-flash."
}
...
let workingBranch = sourceBranch;

if (
  sourceBranch !== branchName &&
  !sourceBranch.startsWith(`${branchName}-`)
) {
  const existing = run("git", ["branch", "--list", branchName], {
    capture: true,
  });

  workingBranch = existing
    ? `${branchName}-${new Date()
        .toISOString()
        .replace(/[-:]/g, "")
        .replace(/\..+/, "")
  ...
  run("git", ["switch", "-c", workingBranch]);
}

console.log(`Branch de origem: ${sourceBranch}`);
console.log(`Branch de trabalho: ${workingBranch}`);

step("Criando backup local");
const backupRoot = backupFiles(requiredFiles);
console.log(`Backup: ${backupRoot}`);
console.log("O backup não será incluído no commit.");
...
$TempPatcher = Join-Path $env:TEMP (
    "orcaly-payment-phase1-" + [Guid]::NewGuid().ToString("N") + ".mjs"
)
...
if ($ExitCode -ne 0) {
    throw "A correção foi interrompida. Revise o erro mostrado acima."
}
