...
function Convert-LegacyRepairScriptsToAsciiEscapes(
    [string]$Node,
    [string]$RepoRoot
) {
    Write-Step "Preservando padroes legados de mojibake sem literais corrompidos"

    $FixerPath = Join-Path $RepoRoot ".orcaly-hardening-local\escape-legacy-mojibake.mjs"
...
'@
    ...
    Invoke-Checked $Node --check $FixerPath
    Invoke-Checked $Node $FixerPath
    ...
    foreach ($Relative in $Targets) {
        Invoke-Checked $Node --check $Relative
        ...
        foreach ($Byte in $Bytes) {
            if ($Byte -gt 127) {
                throw "Caractere nao ASCII ainda existe em $Relative."
            }
        }

        if ($script:RepairedMojibakeFiles -notcontains $Relative) {
            $script:RepairedMojibakeFiles += $Relative
        }
    }

    Write-Host "LEGACY_MOJIBAKE_PATTERNS_ESCAPED_OK=1" -ForegroundColor Green
}
...
$TranscriptPath = Join-Path $ReportDir "hardening-continuacao-v6-$Stamp.txt"
...
try {
    ...
    Write-Host "PORTUGUESE_PLACEHOLDER_FILTER_OK=1" -ForegroundColor Green
...
Write-Step "Scanner de credenciais corrigido - historico Git"
    ...
    Convert-LegacyRepairScriptsToAsciiEscapes $Node $RepoRoot
    ...
    Write-Host "SECURITY_STATIC_SCAN_OK=1" -ForegroundColor Green
    ...
    $EncodingFiles = @(
        ...
        "scripts/corrigir-bugs-qa-criticos-altos.cjs",
        "scripts/corrigir-layout-duplicate-logo-url.cjs",
        "scripts/corrigir-module-group-relatorios.cjs",
        "scripts/corrigir-modulo-placeholder-found-type.cjs",
        "scripts/corrigir-panel-module-icon.cjs",
        "scripts/corrigir-presenca-forcado-scan.cjs",
    ...
    foreach ($EncodingFile in $EncodingFiles) {
        if ($script:RepairedMojibakeFiles -notcontains $EncodingFile) {
            $script:RepairedMojibakeFiles += $EncodingFile
        }
    }
    ...
    Write-Host "ORCALY_HARDENING_V6_READY=1" -ForegroundColor Green
}
catch {
    Write-Host ""
    Write-Host "ORCALY_HARDENING_V6_FAILED=1" -ForegroundColor Red
