...
$InstallerVersion = "V5_CLEAN_PATCH_VALIDATION"
$PatchApplied = $false
$CommitCreated = $false
...
Write-Section "ORCALY - CORREÇÃO APPLICATION_FEE MERCADO PAGO - V5"
...
try {
    ...
    try {
        ...
        if (Test-Path -LiteralPath $ValidationNodeModules) {
            Remove-Item `
                -LiteralPath $ValidationNodeModules `
                -Force `
                -ErrorAction SilentlyContinue
        }
    ...
    Write-Host "Security check: commit limpo + patch"
    Write-Host "Build: commit limpo + patch"
