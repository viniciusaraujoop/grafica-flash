...
function AssertAuditShape([string]$Relative) {
    $path = FullPath $Relative
    ...
    switch ($Relative) {
        ...
        default {
            throw ("Arquivo sem regra de auditoria: " + $Relative)
        }
    }
    ...
    foreach ($anchor in $anchors) {
        if (-not $content.Contains($anchor)) {
            throw (
                $Relative +
                " nao contem um trecho funcional necessario para a FASE 2. " +
                "Trecho ausente: " +
                $anchor +
    ...
    $statusLines = @(& git status --porcelain -- $Relative)

    if ($statusLines.Count -gt 0) {
        $status = [string]$statusLines[0]

        if ($status.StartsWith("??")) {
            Warn ($Relative + " esta untracked, mas a estrutura auditada confere. Ele sera incluido no commit.")
        }
        else {
            throw (
                $Relative +
                ...
                $status +
                ". Nao vou misturar mudancas nao auditadas com a FASE 2."
            )
        }
    }
    else {
        Ok ($Relative + " corresponde a estrutura auditada e nao possui alteracao pendente.")
    }
}
...
try {
    Write-Host ""
    Write-Host "ORCALY - FASE 2 - PERMISSOES GRANULARES V4" -ForegroundColor Cyan
    ...
    foreach ($relative in $AuditedFiles) {
        AssertAuditShape $relative
    }

    $alreadyMarkers = @(
        "portal.access",
        "allowedProspectorPage",
        "sanitizeProspectorPermissions"
    )

    foreach ($marker in $alreadyMarkers) {
        foreach ($relative in $AuditedFiles) {
            $content = ReadText $relative
            if ($content.Contains($marker)) {
                throw (
                    "Foi detectada aplicacao parcial/anterior da FASE 2 em " +
                    $relative +
                    " (" + $marker + "). Interrompendo para evitar duplicacao."
                )
            }
        }
    }
