...
lacks(provider, /(?:authorization|apiKey)\s*[:=][^\n]*(?:AI_GATEWAY_API_KEY|VERCEL_OIDC_TOKEN|OPENAI_API_KEY)/i, 'provider runtime must not manually forward AI credential values')
has(provider, /configuredAuthMode/, 'provider must observe auth mode without exposing credential values')
