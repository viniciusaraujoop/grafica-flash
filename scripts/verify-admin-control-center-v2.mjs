...
assert(supportMode.includes("resolvePlatformFeatureFlag(session.supabaseAdmin, 'support.mode')"), 'Support mode must fail closed behind feature flag')
...
assert(ai.includes("resolvePlatformFeatureFlag(session.supabaseAdmin, 'admin.ai')"), 'Admin AI must fail closed behind feature flag')
