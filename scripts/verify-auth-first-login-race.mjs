...
assert.match(loginPage, /signInWithPasswordFormAction/, 'Login precisa expor a Server Action diretamente ao formulário SSR.')
assert.match(loginPage, /useActionState\(/, 'Login precisa consumir estado da Server Action sem criar um submit client-only.')
assert.match(loginPage, /<form\s+action=\{loginAction\}/, 'Formulário precisa continuar funcional antes da hidratação.')
assert.match(loginPage, /name="email"/, 'Campo de e-mail precisa integrar o FormData server-side.')
assert.match(loginPage, /name="password"/, 'Campo de senha precisa integrar o FormData server-side.')
assert.equal(loginPage.includes('onSubmit={entrar}'), false, 'Login não pode depender de onSubmit hidratado para autenticar.')
assert.equal(loginPage.includes('signInWithPasswordAction({'), false, 'Página cliente não pode chamar a action imperativamente após hidratação.')
...
assert.equal(loginPage.includes('window.location.reload'), false, 'Login não pode usar reload como sincronização de sessão.')
...
assert.match(loginAction, /signInWithPasswordFormAction/, 'Actions precisa oferecer contrato de FormData progressivamente melhorado.')
assert.match(loginAction, /formData\.get\('email'\)/, 'Form action precisa ler e-mail no servidor.')
assert.match(loginAction, /formData\.get\('password'\)/, 'Form action precisa ler senha no servidor.')
