...
try {
    Write-Host ""
    Write-Host `
        "ORCALY - WHATSAPP PEDIDOS DETALHADO V3" `
...
'@
    ...
    $listOldButton =
        $listOldButton -replace "`r?`n", $listNl
...
'@
    ...
    $listNewButton =
        $listNewButton -replace "`r?`n", $listNl

    $listText = ReplaceExactOnce `
        $listText `
        $listOldButton `
        $listNewButton `
        "Botao Chamar no WhatsApp da lista"
...
'@
    ...
    $checks.Add(
        @(
            -not $listAfter.Contains(
                "whatsappLink(order.telefone"
            ),
            "lista nao usa helper antigo"
        )
    )

    $checks.Add(
        @(
            -not $detailAfter.Contains(
                "whatsappLink(order.telefone"
            ),
            "Pedido Pro nao usa helper antigo"
        )
    )
