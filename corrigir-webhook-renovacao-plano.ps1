...
function calcularNovaExpiracao(expiracaoAtual: string | null) {
  const agora = new Date()

  if (!expiracaoAtual) {
    return somarDias(agora, 30)
  }

  const dataExpiracaoAtual = new Date(expiracaoAtual)

  if (Number.isNaN(dataExpiracaoAtual.getTime())) {
    return somarDias(agora, 30)
  }

  if (dataExpiracaoAtual > agora) {
    return somarDias(dataExpiracaoAtual, 30)
  }
