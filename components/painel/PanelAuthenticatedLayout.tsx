        ...
        <div className="w-full overflow-hidden rounded-[2rem] border border-blue-100 bg-white shadow-2xl shadow-blue-950/10">
          <div className="relative overflow-hidden bg-[#05245c] px-6 py-10 text-white sm:px-10">
            ...
            <div className="relative">
              <img src="/logo-orcaly.png" alt="Orçaly" className="mb-8 h-14 w-auto object-contain brightness-0 invert" />
          ...
          <div className="grid gap-5 p-6 sm:grid-cols-3 sm:p-10">
            ...
            <div className="rounded-3xl border border-blue-50 bg-[#f8fbff] p-5">
              ...
              <p className="mt-2 text-xl font-black text-[#071b3a]">{normalizePlano(empresa.assinatura_plano || empresa.plano)}</p>
          ...
          <div className="border-t border-blue-50 bg-white px-6 pb-8 sm:px-10">
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row">
              {podeRenovar ? (
                <Link href="/painel/assinatura" className="inline-flex items-center justify-center rounded-2xl bg-[#05245c] px-6 py-4 text-center font-black text-white shadow-lg shadow-blue-950/20 transition hover:-translate-y-0.5">
                  Renovar assinatura
                </Link>
              ) : (
                <button type="button" disabled className="inline-flex cursor-not-allowed items-center justify-center rounded-2xl bg-slate-200 px-6 py-4 text-center font-black text-slate-500">
                  Renovação disponível apenas para dono/gerente
                </button>
              )}

              {empresa.assinatura_checkout_url && podeRenovar ? (
                <a href={empresa.assinatura_checkout_url} className="inline-flex items-center justify-center rounded-2xl border border-blue-100 bg-white px-6 py-4 text-center font-black text-[#05245c] transition hover:-translate-y-0.5">
                  Continuar pagamento aberto
                </a>
              ) : null}

              <button type="button" onClick={() => router.refresh()} className="inline-flex items-center justify-center rounded-2xl border border-blue-100 bg-white px-6 py-4 text-center font-black text-[#05245c] transition hover:-translate-y-0.5">
...
export default function PanelAuthenticatedLayout({ payload, children }: { payload: PanelAccessPayload; children: ReactNode }) {
