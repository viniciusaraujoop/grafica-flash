...
if (!(Test-Path -LiteralPath "package.json")) {
  Write-Host "package.json nao encontrado. Rode este script na raiz do projeto." -ForegroundColor Red
  exit 1
}

New-Item -ItemType Directory -Force -Path "scripts" | Out-Null

$patcherPath = Join-Path $project "scripts\corrigir-bugs-qa-criticos-altos.cjs"
$patcherContent = @'
const fs = require(''fs'')
...
if ($LASTEXITCODE -ne 0) {
  Write-Host "Falha ao aplicar correções de QA." -ForegroundColor Red
  exit $LASTEXITCODE
}

Write-Host ""
Write-Host "Correções aplicadas. Agora rode:" -ForegroundColor Green
