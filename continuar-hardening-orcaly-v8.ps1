...
$RunDir = Join-Path $ReportDir "v8-$Stamp"
...
$LintJson = Join-Path $RunDir "eslint-v8.json"
...
$LintSummary = Join-Path $RunDir "eslint-v8-summary.txt"
...
$SummaryJson = Join-Path $RunDir "v8-gates.json"
[System.IO.File]::WriteAllText(
    $SummaryJson,
...
Write-Host "V8_REPORT_DIR=$RunDir"
Write-Host "V8_LINT_SUMMARY=$LintSummary"
...
if ($Failed.Count -eq 0) {
    Write-Host "ORCALY_V8_ALL_CODE_GATES_OK=1" -ForegroundColor Green
} else {
    Write-Host "ORCALY_V8_DIAGNOSTICS_COMPLETE=1" -ForegroundColor Yellow
