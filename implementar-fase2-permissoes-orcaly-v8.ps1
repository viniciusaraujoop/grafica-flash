...
$BackupRoot = Join-Path $ProjectRoot (".orcaly-backups\fase2-merge-v8-" + (Get-Date -Format "yyyyMMdd-HHmmss"))
...
function WriteNormalized([string]$Relative, [string]$Text) {
    ...
    try {
        [IO.File]::WriteAllText($tempPath, $normalized, $Utf8NoBom)
        ...
        if (-not $written) {
            ...
            throw (
                "Windows manteve o arquivo bloqueado apos 8 tentativas: " +
                $Relative +
                ". Feche npm run dev/Next.js e qualquer editor que esteja prendendo o arquivo. Detalhe: " +
                $message
            )
        }

        $verify = [IO.File]::ReadAllText($path)
        if ((NormalizeNewlines $verify) -ne $normalized) {
            throw ("Falha ao verificar a escrita final de " + $Relative)
        }
    }
    finally {
...
try {
    Write-Host ""
    Write-Host "ORCALY - FASE 2 - MERGE OWNER/SUPPORT + PROSPECTOR V8" -ForegroundColor Cyan
    ...
    Step "Pre-validando todas as ancoras principais"

    $preflightChecks = @(
        @($platform, "export type PlatformRole = 'owner' | 'admin' | 'finance' | 'support'", "PlatformRole"),
        @($platform, "  { key: 'dashboard.view', label: 'Ver central de suporte', description: 'Acessar o resumo operacional permitido ao suporte.', supportAssignable: true },", "Catalogo portal"),
        @($platform, "const SUPPORT_ALLOWED = new Set<PlatformPermission>([", "SUPPORT_ALLOWED"),
        @($platform, "export function canPlatform(admin: PlatformAdmin, permission: PlatformPermission) {", "canPlatform"),
        @($platform, "export function sanitizeSupportPermissions(value: unknown) {", "sanitizeSupportPermissions"),
        @($platform, "export async function getCurrentPlatformAdminFromRequest(request: NextRequest): Promise<PlatformAdmin | null> {", "getCurrentPlatformAdminFromRequest"),
        @($session, "    'dashboard.view',", "session dashboard.view"),
        @($company, ".from('admin_users')", "company-access admin_users"),
        @($proxy, "      const activeSupport = active && role === 'support'", "proxy activeSupport"),
        @($proxy, "      if (passwordPage && !officialOwner && !activeSupport) {", "proxy password"),
        @($proxy, "      if ((officialOwner || activeSupport) && access?.must_change_password === true && !passwordPage) {", "proxy must_change")
    )

    foreach ($check in $preflightChecks) {
        $haystack = NormalizeNewlines ([string]$check[0])
        $required = NormalizeNewlines ([string]$check[1])
        $label = [string]$check[2]

        if ((CountExact $haystack $required) -ne 1) {
            throw ("Pre-validacao falhou em: " + $label + ". Nenhuma escrita foi iniciada.")
        }

        Ok ("Ancora confirmada: " + $label)
    }
    ...
    if ($nodeProcesses.Count -gt 0) {
        Warn (
            "Ha " +
            $nodeProcesses.Count +
            " processo(s) node.exe em execucao. Se o Windows bloquear a escrita, encerre o npm run dev antes de tentar novamente."
        )
    }
