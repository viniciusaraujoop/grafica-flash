...
# IMPORTANTE:
# PowerShell "desempacota" automaticamente saídas de funções:
# 0 itens -> $null, 1 item -> objeto escalar, 2+ -> array.
# Todas as coleções usadas com .Count são encapsuladas com @(...)
# para manter comportamento consistente sob Set-StrictMode.
...
Write-Section "ORÇALY - COMMIT E DEPLOY DO AVISO MERCADO PAGO - V2"
...
$UnexpectedTracked = @(Test-AllowedScope -Files $TrackedChanges)
...
$UnexpectedExistingStaged = @(Test-AllowedScope -Files $ExistingStaged)
...
$UnexpectedStaged = @(Test-AllowedScope -Files $StagedFiles)
