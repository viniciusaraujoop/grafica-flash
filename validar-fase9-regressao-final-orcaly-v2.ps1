...
function BackupFile([string]$Relative) {
...
function FixProxyCookieType {
    $relative = "proxy.ts"
    $path = FullPath $relative
    $content = ReadText $relative

    $typedImport = "import { createServerClient, type CookieOptions } from '@supabase/ssr'"
    $legacyImport = "import { createServerClient } from '@supabase/ssr'"
    $typedOption = "options?: CookieOptions"
    $legacyOption = "options?: any"

    if ($content.Contains($typedImport) -and $content.Contains($typedOption)) {
        Ok "proxy.ts ja usa CookieOptions."
        return
    }

    if (-not $content.Contains($legacyImport)) {
        throw "Import esperado de createServerClient nao encontrado em proxy.ts. Nao vou adivinhar o merge."
    }

    if (-not $content.Contains($legacyOption)) {
        throw "options?: any esperado nao encontrado em proxy.ts. Nao vou adivinhar o merge."
    }
    ...
    $final = ReadText $relative
    Assert ($final.Contains($typedImport)) "proxy.ts importa CookieOptions"
    Assert ($final.Contains($typedOption)) "proxy.ts remove any de CookieToSet"
    Assert (-not $final.Contains($legacyOption)) "proxy.ts sem any legado em CookieToSet"
}
...
function Rollback {
    ...
    foreach ($relative in $items) {
        if ($Backups.ContainsKey($relative)) {
            ...
            Warn ("Restaurado no rollback: " + $relative)
        } else {
...
try {
    Write-Host ""
    Write-Host "ORCALY - FASE 9 V2 - REGRESSAO FINAL FOUNDER" -ForegroundColor Cyan
    ...
    Step "Corrigindo tipagem SSR legada do proxy"
    FixProxyCookieType
    ...
    AssertContains "proxy.ts" "type CookieOptions" "Proxy usa tipo oficial CookieOptions"
    AssertContains "proxy.ts" "options?: CookieOptions" "CookieToSet sem any"
    AssertNotContains "proxy.ts" "options?: any" "Any legado removido do proxy"
    ...
    Write-Host " - Tipagem SSR do proxy sem explicit any"
    ...
    if ($Changed.Count -gt 0) {
        Warn "Executando rollback dos arquivos alterados pela FASE 9 V2."
        Rollback
    }
