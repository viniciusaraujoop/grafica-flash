...
function AssertBlob([string]$Relative, [string]$Expected) {
    $actual = (& git rev-parse ("HEAD:" + $Relative) 2>$null | Select-Object -First 1)

    if ($LASTEXITCODE -ne 0) {
        throw ("Nao foi possivel ler o blob HEAD de " + $Relative)
    }
    ...
    if ($actual -ne $Expected.ToLowerInvariant()) {
        throw (
            $Relative +
            " mudou desde a auditoria. Esperado " +
            $Expected +
            "; atual " +
            $actual +
            ". Nao vou sobrescrever codigo nao auditado."
        )
    }
...
try {
    Write-Host ""
    Write-Host "ORCALY - FASE 2 - PERMISSOES GRANULARES" -ForegroundColor Cyan
    ...
    foreach ($entry in $ExpectedBlobs.GetEnumerator()) {
        AssertBlob $entry.Key $entry.Value
        AssertClean $entry.Key
    }
