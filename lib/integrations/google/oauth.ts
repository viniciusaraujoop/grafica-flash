...
export const GOOGLE_OAUTH_PROVIDERS = new Set<IntegrationProviderKey>([
  'google_calendar', 'google_drive', 'google_sheets', 'gmail', 'google_business',
])

const identityScopes = ['openid', 'email'] as const
const scopeMap: Partial<Record<IntegrationProviderKey, readonly string[]>> = {
...
export function isGoogleOAuthProvider(provider: IntegrationProviderKey) {
  return GOOGLE_OAUTH_PROVIDERS.has(provider)
}
