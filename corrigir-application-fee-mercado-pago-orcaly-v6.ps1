...
$InstallerVersion = "V6_NATIVE_OUTPUT_AND_JUNCTION_FIX"
$PatchApplied = $false
$CommitCreated = $false
...
Write-Section "ORCALY - CORREÇÃO APPLICATION_FEE MERCADO PAGO - V6"
...
try {
    ...
    try {
        ...
        $RootNodeModules = Join-Path `
            $Root `
            "node_modules"

        if (-not (Test-Path -LiteralPath $RootNodeModules)) {
            throw "node_modules não foi encontrado. Execute npm install antes."
        }
        ...
        try {
            ...
            Write-Section "PREPARANDO DEPENDÊNCIAS PARA O BUILD"

            if (Test-Path -LiteralPath $ValidationNodeModules) {
                $RemoveExistingJunctionCommand = (
                    'rmdir "{0}"' -f
                ...
                if ($LASTEXITCODE -ne 0) {
                    throw "Não foi possível remover a junction anterior de node_modules."
                }
            }

            New-Item `
                -ItemType Junction `
                -Path $ValidationNodeModules `
                -Target $RootNodeModules | Out-Null

            Write-Ok "NODE_MODULES_JUNCTION_CREATED=1"
        ...
        if (Test-Path -LiteralPath $ValidationNodeModules) {
            ...
            if (
                ...
                (Test-Path -LiteralPath $ValidationNodeModules)
            ) {
                Write-Warn "Não foi possível remover automaticamente a junction temporária: $ValidationNodeModules"
            }
            else {
                Write-Ok "NODE_MODULES_JUNCTION_REMOVED=1"
            }
        }
    ...
    Write-Host "Build: commit limpo + patch com junction temporária"
