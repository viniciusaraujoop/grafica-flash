...
function Set-VercelEnv(
    ...
    $argList = @("env", "add", $Name, $Environment, "--force")
    if ($Sensitive) {
        $argList += "--sensitive"
    }
    ...
    if ($process.ExitCode -ne 0) {
        throw "Falha ao gravar $Name na Vercel ($Environment).`n$stderr`n$stdout"
    }
...
if ([string]::IsNullOrWhiteSpace($walletId)) {
    throw "A autenticação funcionou, mas o walletId não foi encontrado."
}
...
if ($asaasEnvironment -eq "production") {
    ...
    Write-Host "Sandbox salvo apenas em Preview. Production permaneceu intacta." -ForegroundColor Yellow
}

Step "Conferindo variáveis"
& $Vercel env ls $vercelEnvironment
